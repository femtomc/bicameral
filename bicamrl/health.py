...
class HealthChecker:
    ...
    async def check_memory(self, memory) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unhealthy", "error": str(e), "message": "Memory system error"}
    ...
    async def check_storage(self, memory) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unhealthy", "error": str(e), "message": "Storage check error"}

    async def check_sleep_layer(self, sleep_layer) -> Dict[str, Any]:
        """Check Sleep layer health."""
        if not sleep_layer:
            return {"status": "disabled", "message": "Sleep layer not configured"}
        ...
        try:
            return {
                ...
                "queue_size": (
                    sleep_layer.observation_queue.qsize()
                    if hasattr(sleep_layer, "observation_queue")
                    else 0
                ),
                "insights_cached": (
                    len(sleep_layer.insights_cache) if hasattr(sleep_layer, "insights_cache") else 0
                ),
                "providers": list(sleep_layer.llms.keys()) if hasattr(sleep_layer, "llms") else [],
                "message": (
                    "Sleep layer operational" if sleep_layer.is_running else "Sleep layer stopped"
                ),
            }
        except Exception as e:
            logger.error(f"Sleep layer health check failed: {e}")
            return {"status": "unhealthy", "error": str(e), "message": "Sleep layer error"}

    async def check_all(
        self, memory, sleep_layer, pattern_detector, feedback_processor
    ) -> Dict[str, Any]:
        ...
        components = {
            ...
            "pattern_detector": {
                ...
                "message": (
                    "Pattern detector operational" if pattern_detector else "Not initialized"
                ),
            },
            "feedback_processor": {
                "status": "healthy" if feedback_processor else "not_initialized",
                "message": (
                    "Feedback processor operational" if feedback_processor else "Not initialized"
                ),
            },
        }
