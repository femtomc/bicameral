...
from ..core.interaction_model import Action, ActionStatus, Interaction
...
class ClaudeCodeImporter:
    ...
    async def import_directory(
        self, claude_dir: Path = None, project_filter: Optional[str] = None
    ) -> Dict[str, int]:
    ...
    async def import_conversation_log(self, log_path: Path) -> Dict[str, int]:
        ...
        stats = {"interactions": 0, "actions": 0, "patterns": 0}
        ...
        for line in lines:
            try:
                ...
                if event_type == "user":
                    ...
                    current_interaction = self._create_interaction_from_user_event(
                        event, session_id
                    )
    ...
    def _create_interaction_from_user_event(self, event: Dict, session_id: str) -> Interaction:
        ...
        if timestamp_str:
            timestamp = datetime.fromisoformat(timestamp_str.replace("Z", "+00:00"))  # Let it fail!
        else:
        ...
        return Interaction(
            ...
            user_query=content,
            timestamp=timestamp,
            query_context={"cwd": event.get("cwd", ""), "version": event.get("version", "")},
            actions_taken=[],
    ...
    def _parse_tool_use(self, tool_use: Dict) -> Optional[Action]:
        ...
        return Action(
            ...
            target=target,
            details={"tool_name": tool_name, "tool_input": tool_input, "tool_use_id": tool_use_id},
    ...
    async def _save_interaction(self, interaction: Interaction):
        ...
        try:
            ...
            failed_actions = [
                a for a in interaction.actions_taken if a.status == ActionStatus.FAILED
            ]
