...
3. Impact assessment""",
    ...
    async def optimize_prompt(
        ...
        if template:
            optimized = await self._apply_template(
                template, prompt, analysis, patterns, preferences
            )
        else:
    ...
    def _extract_entities(self, prompt: str) -> Dict[str, List[str]]:
        ...
        entities = {"files": [], "functions": [], "variables": [], "technologies": []}
    ...
    def _find_similar_interactions(
        self, prompt: str, recent_interactions: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for action in recent_interactions.get("recent_actions", []):
            if "action" in action and isinstance(action["action"], str):
                ...
                if similarity > 0.3:
                    similar.append({"action": action, "similarity": similarity})
    ...
    def _select_template(
        self, analysis: Dict[str, Any], task_type: Optional[str]
    ...
    async def _enhance_prompt(
        self, prompt: str, patterns: List[Dict[str, Any]], preferences: Dict[str, Any]
    ) -> str:
    ...
    async def _select_context(
        ...
        for file in analysis["entities"].get("files", []):
            context_items.append({"type": "file", "path": file, "reason": "mentioned in prompt"})
    ...
    async def learn_from_interaction(
        self, prompt: str, optimized_prompt: str, outcome: Dict[str, Any]
    ):
    ...
    def _add_role_context(
        self, context_additions: List[Dict[str, Any]], role: CommandRole
    ) -> List[Dict[str, Any]]:
