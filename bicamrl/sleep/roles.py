...
class CommandRole(BaseModel):
    ...
    mindset_prompt: Optional[str] = Field(
        None, description="Optimized prompt to induce behavioral mindset"
    )
    domain: Optional[str] = Field(None, description="Primary domain this role operates in")
    discovered_from_pattern: Optional[str] = Field(
        None, description="Pattern ID this role was discovered from"
    )

    # Activation
    context_triggers: List[ContextTrigger] = Field(
        default_factory=list, description="Conditions that activate this role"
    )
    confidence_threshold: float = Field(0.7, description="Minimum confidence to activate role")

    # Behavior
    tool_preferences: Dict[str, float] = Field(
        default_factory=dict, description="Tool name -> preference weight"
    )
    decision_rules: List[DecisionRule] = Field(
        default_factory=list, description="Rules guiding behavior"
    )
    ...
    successful_patterns: List[Dict[str, Any]] = Field(
        default_factory=list, description="Patterns that worked well"
    )
    ...
    def to_prompt_context(self) -> str:
        ...
        # Use mindset prompt if available (from LLM role discovery)
        if self.mindset_prompt:
            lines = [self.mindset_prompt, ""]
        else:
            lines = [
    ...
    def to_markdown(self) -> str:
        ...
        if self.successful_patterns:
            lines.extend(
                ["## Successful Patterns", "", "Patterns that have worked well with this role:", ""]
            )
