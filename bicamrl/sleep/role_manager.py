...
from .roles import CommandRole, BUILTIN_ROLES
from .role_discoverer import RoleDiscoverer
from .interaction_role_discoverer import InteractionRoleDiscoverer
...
class RoleManager:
    ...
    def __init__(self, memory: Memory, hybrid_store: Optional[HybridStore] = None, config: Optional[Dict[str, Any]] = None):
        self.memory = memory
        ...
        self.storage_path = Path(roles_config.get('storage_path', '~/.bicamrl/roles.json')).expanduser()
        
        # Always use interaction-based discoverer
        self.discoverer = InteractionRoleDiscoverer(memory, hybrid_store)
        self.use_interaction_discovery = True
                ...
                for role_summary in data.get('custom_roles', []):
                    ...
                    if md_file:
                        ...
                        if md_path.exists():
                            ...
                            role_data_file = roles_dir / f"{role_name.lower().replace(' ', '_')}.json"
                            if role_data_file.exists():
                                ...
                                logger.warning(f"Skipping role {role_name} - no full data available")
                                continue
    ...
    async def get_active_role(self, context: Dict[str, Any]) -> Optional[CommandRole]:
        ...
        if self.auto_discover and (datetime.now() - self.last_discovery).seconds > self.discovery_interval:
    ...
    def _log_role_transition(self, from_role: Optional[CommandRole], to_role: CommandRole, context: Dict[str, Any]):
    ...
    async def discover_new_roles(self):
        ...
        logger.info(f"Starting role discovery process (using {'interaction' if self.use_interaction_discovery else 'action'} discovery)")
        self.last_discovery = datetime.now()
        
        try:
            # Use appropriate discovery method
            if self.use_interaction_discovery:
                # Set minimum interactions on discoverer if it supports it
                if hasattr(self.discoverer, 'min_interactions_per_role'):
    ...
    def _is_duplicate_role(self, new_role: CommandRole) -> bool:
        ...
        for existing_role in all_roles.values():
            ...
            existing_triggers = {(t.trigger_type, t.pattern) for t in existing_role.context_triggers}
