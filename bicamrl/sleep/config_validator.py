...
class SleepConfigValidator:
    ...
    @classmethod
    def _validate_llm_provider(cls, name: str, config: Dict[str, Any], 
                              errors: List[str], warnings: List[str]) -> Optional[Dict[str, Any]]:
        ...
        if name == 'claude':
            if 'api_key' not in config and not config.get('use_env', True):
                errors.append(f"Claude provider requires api_key or use_env=true")
                return None
                
        elif name == 'openai':
            if 'api_key' not in config and not config.get('use_env', True):
                errors.append(f"OpenAI provider requires api_key or use_env=true")
                return None
        ...
        for field, field_type in cls.LLM_PROVIDER_FIELDS.items():
            ...
            if field in config:
                if not isinstance(config[field], expected_type):
                    errors.append(f"Provider {name} field {field} must be of type {expected_type.__name__}")
                else:
                    validated[field] = config[field]
            elif default is not None:
                validated[field] = default
