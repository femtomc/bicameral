...
from ..core.interaction_model import Interaction, FeedbackType
...
class HybridStore:
    ...
    async def search_similar_queries(self, 
                                   query: str, 
                                   k: int = 5,
    ...
    async def search_similar_patterns(self, 
                                    pattern: str,
    ...
    async def find_correction_patterns(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        all_embeddings = await self.vector_store.search_similar(
            ...
            k=1000,  # Get many
            threshold=-1.0  # Get all
        )
        
        # Filter for corrections
        corrections = []
        for emb_id, _, metadata in all_embeddings:
            if (metadata.get('type') == 'user_feedback' and 
                metadata.get('feedback_type') == FeedbackType.CORRECTION.value):
    ...
    async def find_successful_patterns(self, 
        ...
        successful = []
        for emb_id, _, metadata in all_embeddings:
            if (metadata.get('type') == 'user_query' and 
    ...
    async def cluster_similar_queries(self, min_cluster_size: int = 3, similarity_threshold: float = 0.7) -> List[List[Dict[str, Any]]]:
    ...
    async def get_feedback_statistics(self) -> Dict[str, Any]:
        ...
        for interaction in interactions:
            ...
            if interaction_data.get('feedback_type') and interaction_data.get('feedback_type') != 'none':
            ...
            if 'details' in interaction_data and isinstance(interaction_data['details'], dict):
                ...
                if 'interaction_id' in details and 'user_query' in details:
                    try:
                        ...
                        existing = await self.vector_store.get_embedding(f"query_{interaction.interaction_id}")
                        if existing is None:
                            # Add embeddings
