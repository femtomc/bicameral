...
class VectorStore:
    ...
    async def add_embedding(
        self, embedding_id: str, embedding: np.ndarray, metadata: Optional[Dict[str, Any]] = None
    ):
    ...
    async def batch_add_embeddings(self, embeddings: List[Tuple[str, np.ndarray, Dict[str, Any]]]):
    ...
    def statistics(self) -> Dict[str, Any]:
        ...
        if self.embeddings is None:
            return {"count": 0, "dimension": 0, "storage_size_mb": 0}
...
class LanceDBVectorStore(VectorStore):
    ...
    async def search_similar(
        ...
        results = self.table.search(query_vec).limit(k).to_pandas()
        ...
        for _, row in results.iterrows():
            ...
            if similarity >= threshold:
                ...
                output.append((row["id"], float(similarity), metadata))
...
class ChromaDBVectorStore(VectorStore):
    ...
    def __init__(self, storage_path: Path):
        ...
        try:
            ...
            self.collection = self.client.get_or_create_collection(
                name=self.collection_name, metadata={"description": "Bicamrl embeddings storage"}
            )
    ...
    async def search_similar(
        ...
        # Query collection
        results = self.collection.query(query_embeddings=[query_list], n_results=k)
        ...
        if results["ids"] and len(results["ids"][0]) > 0:
            for i in range(len(results["ids"][0])):
                ...
                if similarity >= threshold:
                    ...
                    output.append((embedding_id, float(similarity), metadata))
    ...
    async def get_embedding(self, embedding_id: str) -> Optional[Tuple[np.ndarray, Dict[str, Any]]]:
        ...
        # Get by ID
        results = self.collection.get(ids=[embedding_id], include=["embeddings", "metadatas"])
    ...
    async def batch_add_embeddings(self, embeddings: List[Tuple[str, np.ndarray, Dict[str, Any]]]):
        ...
        # Batch add
        self.collection.add(
            embeddings=embedding_lists, ids=ids, metadatas=metadatas, documents=documents
        )
