...
class SQLiteStore:
    ...
    def _init_db(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='interactions'")
                if cursor.fetchone():
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_world_models_session ON world_model_states(session_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_world_models_domain ON world_model_states(domain)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_world_models_active ON world_model_states(is_active)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_world_snapshots_model ON world_model_snapshots(world_model_id)')
    ...
    async def add_complete_interaction(self, interaction_dict: Optional[Dict[str, Any]] = None, **kwargs) -> None:
    ...
    async def add_world_model_state(self, world_state: Dict[str, Any]) -> str:
        ...
        def _add():
            with sqlite3.connect(self.db_path) as conn:
                ...
                entities_json = json.dumps([
                    {
                        ...
                        'first_seen': e.first_seen.isoformat() if hasattr(e.first_seen, 'isoformat') else e.first_seen,
                        'last_modified': e.last_modified.isoformat() if hasattr(e.last_modified, 'isoformat') else e.last_modified
                    }
                ...
                goals_json = json.dumps(world_state.get('inferred_goals', []))
                metrics_json = json.dumps(world_state.get('metrics', {}))
                entity_types_json = json.dumps(list(world_state.get('discovered_entity_types', set())))
                relation_types_json = json.dumps(list(world_state.get('discovered_relation_types', set())))
                constraints_json = json.dumps(world_state.get('constraints', {}))
    ...
    async def get_world_model_state(self, world_model_id: str) -> Optional[Dict[str, Any]]:
        ...
        def _get():
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    'SELECT * FROM world_model_states WHERE id = ?',
                    (world_model_id,)
                )
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    result['discovered_entity_types'] = set(json.loads(result['discovered_entity_types']))
                    result['discovered_relation_types'] = set(json.loads(result['discovered_relation_types']))
    ...
    async def get_active_world_models(self, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def add_world_model_snapshot(self, world_model_id: str, snapshot_data: Dict[str, Any], trigger_event: str) -> None:
