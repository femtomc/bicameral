...
class PatternDetector:
    ...
    async def check_for_patterns(self) -> List[Dict[str, Any]]:
        ...
        logger.info(
            ...
            extra={"interaction_count": len(interactions), "session_id": context.get("session_id")},
        )
        ...
        confident_patterns = [
            p for p in patterns if p.get("confidence", 0) >= self.confidence_threshold
        ]

        duration = time.time() - start_time
        logger.info(
            "Pattern detection completed",
            extra={"patterns_found": len(confident_patterns), "duration_seconds": duration},
        )
...
Return the indices of the most relevant patterns with relevance scores."""
    ...
    def _fallback_similarity_search(
        self, query: str, patterns: List[Dict[str, Any]], limit: int
    ) -> List[Dict[str, Any]]:
