...
from .interaction_model import Interaction, InteractionPattern
...
class InteractionPatternDetector:
    ...
    async def _detect_success_patterns(
        ...
        if len(successful) >= self.min_frequency:
            ...
            common_keywords = [
                kw for kw, count in keyword_freq.most_common(10) if count >= self.min_frequency
            ]
    ...
    def _extract_common_phrases(self, queries: List[str]) -> List[str]:
        ...
        # Return common ones
        common = [
            ngram for ngram, count in ngram_freq.most_common(20) if count >= len(queries) * 0.3
        ]  # Appears in 30% of queries
