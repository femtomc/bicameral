...
from .interaction_model import Action, ActionStatus, FeedbackType, Interaction
...
class InteractionLogger:
    ...
    def complete_action(
        self, action: Action, result: Optional[str] = None, error: Optional[str] = None
    ):
    ...
    def complete_interaction(
        ...
        logger.info(
            f"Completed interaction {self.current_interaction.interaction_id} "
    ...
    async def _store_interaction(self, interaction_data: Dict[str, Any]):
        ...
        if interaction_data:
            ...
            logger.warning("No interaction data to store")

    def _get_current_context(self) -> Dict[str, Any]:
        """Get current context (files, recent actions, etc.)."""
        # This will be populated by the actual MCP server
        return {"timestamp": datetime.now().isoformat(), "session_id": self.session_id}
