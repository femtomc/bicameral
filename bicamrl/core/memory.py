...
from ..utils.log_utils import async_log_context, log_memory_operation
...
class Memory:
        ...
        try:
            ...
            if llm_embeddings:
                ...
                self.hybrid_store = HybridStore(
                    self.db_path / "hybrid", llm_embeddings, vector_backend=vector_backend
                )
                logger.debug(
    ...
    def set_llm_provider(self, llm_provider):
        ...
        logger.info(
            ...
            extra={"provider_type": type(llm_provider).__name__, "session_id": self.session_id},
        )
    ...
    async def store_interaction(self, interaction):
        ...
        if self.hybrid_store:
            async with async_log_context(logger, "hybrid_store", interaction_id=interaction_id):
                ...
                logger.info(
                    "Interaction stored in hybrid store",
    ...
    async def search(self, query: str) -> List[Dict[str, Any]]:
        ...
        for pref in preferences:
            if query_lower in pref["key"].lower() or query_lower in str(pref["value"]).lower():
                results.append(
                    {
                        "type": "preference",
                        "name": pref["key"],
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        top_files = sorted(file_counts.keys(), key=lambda x: file_counts[x], reverse=True)[:10]
    ...
    async def get_memory_insights(self, context: str) -> Dict[str, Any]:
        ...
        for pattern in patterns:
            if (
                context_lower in pattern.get("name", "").lower()
                or context_lower in pattern.get("description", "").lower()
            ):
    ...
    async def cluster_similar_queries(
        ...
        if not self.hybrid_store:
            return []
        return await self.hybrid_store.cluster_similar_queries(
    ...
    async def get_memory_stats(self) -> Dict[str, int]:
        ...
        memory_counts = {"active": 0, "working": 0, "episodic": 0, "semantic": 0}
